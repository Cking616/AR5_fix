    ...
    for (counter = 0; counter < MAX_AXES; counter++) {
        ...
        switch (pCiA402Axis->i16State) {
            ...
            case STATE_READY_TO_SWITCH_ON:
                ...
                if (((ControlWord6040 & CONTROLWORD_COMMAND_QUICKSTOP_MASK) == CONTROLWORD_COMMAND_QUICKSTOP) || ((ControlWord6040 & CONTROLWORD_COMMAND_DISABLEVOLTAGE_MASK) == CONTROLWORD_COMMAND_DISABLEVOLTAGE)) {
            ...
            case STATE_SWITCHED_ON:
                ...
                } else if (((ControlWord6040 & CONTROLWORD_COMMAND_QUICKSTOP_MASK) == CONTROLWORD_COMMAND_QUICKSTOP || (ControlWord6040 & CONTROLWORD_COMMAND_DISABLEVOLTAGE_MASK) == CONTROLWORD_COMMAND_DISABLEVOLTAGE)) {
        ...
        if ((ControlWord6040 & CONTROLWORD_COMMAND_ENABLEOPERATION_MASK) == CONTROLWORD_COMMAND_ENABLEOPERATION)  //ECAT_MOD
        {
...
void CiA402_DummyMotionControl(TCiA402Axis *pCiA402Axis) {
    ...
    if (pCiA402Axis->bAxisFunctionEnabled &&
        ...
        if ((pCiA402Axis->Objects.objSoftwarePositionLimit.i32MaxLimit > pCiA402Axis->Objects.objPositionActualValue || pCiA402Axis->Objects.objPositionActualValue > pCiA402Axis->Objects.objTargetPosition) &&
            (pCiA402Axis->Objects.objSoftwarePositionLimit.i32MinLimit < pCiA402Axis->Objects.objPositionActualValue || pCiA402Axis->Objects.objPositionActualValue < pCiA402Axis->Objects.objTargetPosition)) {
...
void CiA402_Application(TCiA402Axis *pCiA402Axis) {
    ...
    if (bDcSyncActive && (pCiA402Axis->u32CycleTime != 0) && ((pCiA402Axis->Objects.objSupportedDriveModes >> (pCiA402Axis->Objects.objModesOfOperation - 1)) & 0x01))  //Mode of Operation (0x6060) - 1 specifies the Bit within Supported Drive Modes (0x6502)
    {
...
UINT8 Write0xF030(UINT16 index, UINT8 subindex, UINT32 dataSize, UINT16 MBXMEM *pData, UINT8 bCompleteAccess) {
    ...
    for (i = subindex; i <= lastSubindex; i++) {
        ...
        if (i == 0) {
            ...
            if ((sConfiguredModuleIdentList.aEntries[(i - 1)] != (UINT32)CSV_CSP_MODULE_ID) && (sConfiguredModuleIdentList.aEntries[(i - 1)] != (UINT32)CSP_MODULE_ID) && (sConfiguredModuleIdentList.aEntries[(i - 1)] != (UINT32)CSV_MODULE_ID) && (sConfiguredModuleIdentList.aEntries[(i - 1)] != 0)) {
...
void APPL_InputMapping(UINT16 *pData) {
    ...
    for (j = 0; j < sTxPDOassign.u16SubIndex0; j++) {
        ...
        LocalAxes[AxisIndex].Objects.objPositionActualValue = (int)(gsM1_Drive.sPositionEnc.f32PositionMech / 360 * MAGNET_ENCODER_BITS * HARMONIC_AMPLIFY) & 0xFFFFFFFF;  //ECAT_MOD
        LocalAxes[AxisIndex].Objects.objVelocityActualValue = (int)gsM1_Drive.sSpeed.f32SpeedFilt * MAGNET_ENCODER_BITS / 60;
...
void APPL_OutputMapping(UINT16 *pData) {
    ...
    for (j = 0; j < sRxPDOassign.u16SubIndex0; j++) {
        ...
        /*1600 1610  1620*/
        switch ((sRxPDOassign.aEntries[j] & 0x000F)) {                                /*1600 1601 1602*/
