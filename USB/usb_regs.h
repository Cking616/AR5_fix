...
uint32_t Reserved0 :
    1;
uint32_t modemismatch :    //模式不匹配中断
    1;
uint32_t otgintr :   //OTG 中断
    1;
uint32_t sofintr :   //帧起始
    1;
uint32_t rxstsqlvl :   //RxFIFO 非空
    1;
uint32_t nptxfempty :   //非周期性 TxFIFO 空
    1;
uint32_t ginnakeff :    //全局非周期性 IN NAK 有效
    1;
uint32_t goutnakeff :   //全局 OUT NAK 有效
    1;
uint32_t Reserved8_9 :
    2;
uint32_t erlysuspend :   //ESUSP：早期挂起
    1;
uint32_t usbsuspend :   //USBSUSP：USB 挂起 (
    1;
uint32_t usbreset :
    1;
uint32_t enumdone :   //ENUMDNE：枚举完成 (Enumeration
    1;
uint32_t isooutdrop :   //丢弃同步 OUT 数据包中断
    1;
uint32_t eopframe :   //周期性帧结束中断
    1;
uint32_t Reserved16 :
    1;
uint32_t epmismatch :   //
    1;
uint32_t inepintr :   //IN 端点中断 (IN
    1;
uint32_t outepintr :   //OUT 端点中断 (
    1;
uint32_t incomplisoin :   //未完成 IN 同步传输
    1;
uint32_t incomplisoout :  //未完成周期性传输 (In
    1;
uint32_t Reserved22_23 :
    2;
uint32_t portintr :   //主机端口中断 (Host
    1;
uint32_t hcintr :  //主机通道中断 (Ho
    1;
uint32_t ptxfempty :   //周期性 TxFIFO 空
    1;
uint32_t Reserved27 :
    1;
uint32_t conidstschng : //CIDSCHG：连接器 ID 线状态更改
    1;
uint32_t disconnect :  //检测到断开连接中断
    1;
uint32_t sessreqintr :  //：检测到会话请求/新会话中
    1;
uint32_t wkupintr :   //检测到恢复/远程唤醒中断
    1;
  }
  b;
