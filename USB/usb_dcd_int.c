...
uint32_t DCD_HandleUsbReset_ISR(USB_OTG_CORE_HANDLE *pdev);
...
uint32_t USBD_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
    ...
    {
        gintr_status.d32 = USB_OTG_ReadCoreItr(pdev);  //0x014
        if (!gintr_status.d32) /* avoid spurious interrupt */
        {
            return 0;
        }
        if (gintr_status.b.inepint)   //IN端点中断
        {
            USB_OTG_DIEPINTn_TypeDef  diepint;
        ...
        if (gintr_status.b.outepintr) //OUT端点中断
        {
            uint32_t ep_intr;
            USB_OTG_DOEPINTn_TypeDef  doepint;
            USB_OTG_DEPXFRSIZ_TypeDef  deptsiz;
            ...
            while ( ep_intr )
            {
                if (ep_intr & 0x1)    //IN0
                {
                    doepint.d32 = USB_OTG_ReadDevOutEP_itr(pdev, epnum);  //OUT端点中断寄存器 xB08
        ...
        {
            gintr_status.b.nptxfempty=0;
        }

        if (gintr_status.b.ginnakeff)
        {
            gintr_status.b.ginnakeff=0;
        }
